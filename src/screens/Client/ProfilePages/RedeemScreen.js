...
export class RedeemScreen extends Component {
  constructor(props, context) {
    ...
    this.state={
      ...
      cardHolderName: "Emerson Dokidis",
      cardNumber: "4225976500086141",
      expireDate: "08/24",
      CVV: "722",
    }
  ...
  renderLabel = () => (
    <View style={[{backgroundColor: "#2898FF", borderRadius: 3}, S.P5, S.PH10]}>
      <Text style={[S.CLW, S.F14]}>{`$${this.state.amount}`}</Text>
    </View>
  )
  renderNotch = () => (
  ...
  render() {
    const {cardHolderName, cardNumber, expireDate, CVV} = this.state;
    return (
      <Container>
            ...
            <ScrollView horizontal style={[S.MT10, S.PB5]}>
              {
                this.state.paymentMethods.map((item, i) => (
                  <TouchableOpacity onPress={() => this.setState({activePayment: item.key})} key={i} style={[S.ROW, S.Acenter, S.P10, S.PH15, {borderRadius: 5}, S.MH10, this.state.activePayment === item.key ? {borderWidth: 1, borderColor: COLOR.blueColor, backgroundColor: COLOR.lightBlueColor} : {}]}>
                    <View style={[S.ROW]}>
                      {
                        item.icon ? 
                    ...
                    {
                      this.state.activePayment === item.key ?
            ...
            <Form style={[S.MB40]}>
              ...
              <Item style={[{borderColor:COLOR.transparent}, S.ML5]} stackedLabel>
                  ...
                  <Input
                      ...
                      value={cardNumber}
                      onChangeText={e=>this.setState({cardNumber: e})}
                  />
              </Item>
              ...
              <TouchableOpacity style={[S.P15, S.BKB, {borderRadius: 10}, S.MT20]} onPress={()=>navigate("RedeemConfirmScreen")}>
